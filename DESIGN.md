...
A separate practice mode for building *speed* at facts a level's untimed quiz doesn't push on. Reached via a Gym card on Home or the QuickNav's 🧠 tab. The hub (`renderGym()`) is a 6-tile grid, one color each (Tens & Ones pinned first, full-width), plus a picker of toggleable fact-set chips (`FACT_SETS` in `mentalmath.js` — add-to-20, take-away, bridging ten, doubles, make ten, nines, missing-number, times tables, same-tens, and `tensOnes` itself) that filters drill/flash-card fact selection below:
