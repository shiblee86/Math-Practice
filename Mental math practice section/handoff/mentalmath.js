...
const GEN = {
  ...
  doubles() {
    const n = r(3, 9);
    const flavor = pick(['double', 'near', 'nearDown']);
  ...
  // 17 − ? = 10 · ? − 5 = 3 · 10 + ? = 17 · ? + 5 = 9
  missing() {
    const flavor = pick(['subB', 'subA', 'addB', 'addA']);
...
function trainerFact() {
  const kind = pick(['subBridge', 'addBridge', 'missing', 'sameTens', 'countUp', 'countOn']);
  ...
  const mflavor = pick(['subB', 'addB', 'subA']);
...
function flashDeck(cards, session, sets, count) {
  ...
  while (deck.length < count) {
    const f = randFact(pick(sets && sets.length ? sets : ALL_SET_IDS));
