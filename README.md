...
A browser-based math practice app for kids — 20 auto-generated practice levels, 56 real-world "SOAR" activities, trophies/badges/streaks/daily bonus, and save/load progress. Built as a plain static site: no build step, no npm, no framework.
...
- **Unit** — `mathdata.js`'s content and logic in isolation (levels, question generators, SOAR data, trophies/badges).
- **Integration** — the real app, loaded in an iframe and driven like a player would (click through screens, answer every question kind, complete a level, complete a SOAR activity, save/load a file).
...
- Content (levels, word problems, hints, SOAR activities, trophies, badges) lives in `mathdata.js`; interaction/rendering lives in `script.js`; keep that split.
